		...
		<>
			<div className="grid grid-cols-2 gap-5">
				...
				<div className="flex flex-col justify-between">
					<div className="bg-teal-700 text-white font-bold w-fit p-3 rounded-lg mb-5">Nota Fiscal</div>
					<div className="flex flex-col gap-3">
						<p className="font-bold">{item.name}</p>
						<p className="font-bold text-xs">{item.description}</p>
						<p className="font-bold">Valor de aquisição: R$ {item.value}</p>
					</div>
					<OrderButton className="bg-green-600 text-white" itemId={item.id} requesterId={currentUser.id}>Pedir</OrderButton>
				</div>
			</div>
		</>
	);
