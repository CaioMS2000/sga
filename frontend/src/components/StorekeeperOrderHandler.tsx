...
export default function StorekeeperOrderHandler({
	order,
	user,
}: StorekeeperOrderHandlerProps) {
	...
	const DeliveryStatus = (
		<span
			className={`font-bold text-[1.3rem] text-${
	...
	return (
		<>
			<div className="bg-zinc-800 rounded-lg h-fit p-3 text-2xl text-center flex flex-col">
				<h3 className="font-bold mb-5 text-white rounded-lg">
					Almoxarife
				</h3>
				{delivery && (
					<>
						<span className="flex">
							<p className="text-[1.3rem] mr-1">Status:</p>
							{DeliveryStatus}
						</span>
					</>
				)}
				{deliveryStatus != _Status.Concluded && (
					<button
						className="btn bg-purple-700 hover:bg-purple-900 border-purple-700 hover:border-purple-900 text-white font-bold mt-5"
