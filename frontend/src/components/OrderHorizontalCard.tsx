...
export default async function OrderHorizontalCard({
	order: { code, item, analysis, requester, createdAt },storeKeeperView = false
  ...
  const DeliveryStatus = <span className={`text-${deliveryStatus == _Status.Waiting?'orange-500':(deliveryStatus == _Status.Separation?'yellow-500':(deliveryStatus == _Status.InProgress)?'blue-500':'green-500')}`}>{statusToText(deliveryStatus)}</span>;
